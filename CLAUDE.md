...
- **문서 위치와 읽는 시점은 [`docs/README.md`](docs/README.md) 의 표를 따른다.**
  폴더 번호는 **ID 이지 순서가 아니다** — 한 번 정하면 바꾸지 않는다. 각 폴더의
  `README.md` 에 "여기 넣는 것 / 넣지 않는 것 / 지금 있는 문서" 가 있으니, 새
  문서를 만들기 전에 그것부터 본다.
- **작업 시작 전 [`docs/01-decisions/`](docs/01-decisions/) 에서 관련 ADR 을 먼저
  확인한다.** 코드는 "무엇" 만 말하고 "왜" 는 말하지 않는다. 이유를 모른 채 고치면
  좋은 뜻으로 예전 문제를 되살린다. 오래 갈 결정을 새로 내렸으면 `0000-template.md`
  를 복사해 남긴다 — `트레이드오프` 와 `재검토 조건` 칸을 비워 두지 않는다.
- **[`docs/99-reference/`](docs/99-reference/) 는 레거시 분석 자료이며 현재 코드가
  아니다.** 그 폴더의 문서는 제목 바로 다음 줄에
  `> ⚠️ LEGACY — 현재 코드의 동작이 아님. 참고용. (분석일: YYYY-MM-DD)` 를 단다.
  **거기 적힌 것을 이 저장소의 사실로 인용하지 않는다** — 그대로 구현하는 것이
  이 폴더에서 가장 흔한 사고다.
- **코드를 바꾼 변경에서 문서도 같이 고친다.** 썩은 문서는 없는 문서보다 나쁘다.
