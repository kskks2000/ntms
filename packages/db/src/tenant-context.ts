 ...
 * 회사코드로 테넌트를 찾은 다음, 그 테넌트의 user_account 를 조회해서
 * 비밀번호를 확인하기 전까지가 그 구간이다. RLS 는 app.tenant_id 만 보므로
 * 이 상태에서도 테넌트 격리는 그대로 유지된다.
 *
 * app.user_id 가 없으므로 이 트랜잭션에서 만든 행은 created_by 가 NULL 로 남는다.
 * 계정 신청(신청자에게 아직 계정이 없다)처럼 행위자를 특정할 수 없는 경우
 * 외에는 쓰지 않는다. 나머지 쓰기는 사용자가 확정된 뒤 withTenant 로 한다.
 */
export async function withTenantBootstrap<T>(
  prisma: PrismaClient,
  ctx: { tenantId: bigint | number | string; clientIp?: string },
  fn: (tx: TxClient) => Promise<T>,
  options?: { timeout?: number; maxWait?: number },
): Promise<T> {
  const tenantId = toSafeBigInt(ctx.tenantId, 'tenantId');
