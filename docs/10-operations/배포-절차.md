...
**처음 해 보는 사람은 [배포-따라하기.md](./배포-따라하기.md) 부터** — 로컬 리허설부터
서버 배포 · 되돌리기까지 단계마다 정상 출력을 붙여 두었다.
모니터링은 [모니터링.md](./모니터링.md), 전체 그림은
[인수인계 문서](../handoff.md) 에 있다.

**마지막 배포: 2026-09-15 커밋 `6857124`** (`deploy.sh` 6단계 전부 통과, 마이그레이션
대기 0건. 문서만 바뀐 배포라 이미지는 2026-09-14 `2ce6bae` 빌드 그대로다). 정산 시드 적재는 2026-08-22 에 끝냈다 — `seed.js` 로 부대비 유형과 테넌트
사업자등록번호를 채운 뒤 `seed-demo.js --reset`.

> 이 배포 직전 서버 폴더는 `a0d5ad0` 까지 pull 되어 있었지만 **api · web 컨테이너는
> 2026-08-22 에 만든 것**이었다. 소스만 받고 다시 빌드하지 않은 채 3주가 지났다.
> 그래서 "마지막 배포" 는 서버의 `git log` 가 아니라 컨테이너 생성 시각으로 확인한다:
> `docker inspect -f '{{.Created}}' ntms-api`

이 줄은 잘 썩는다. 지금 무엇이 올라가 있는지는 언제나 서버에 직접 묻는다.

```bash
ssh ntms 'cd /opt/ntms && git log --oneline -1'
```
...
- 관리자 로그인은 테넌트 `NTMS` · 계정 `admin`. **비밀번호는 저장소에 적지 않는다** —
  이 저장소는 공개다. 시드 기본 비밀번호가 저장소에 있어 가동계에서는 바꿔 두었고,
  바꾼 값은 저장소 밖(비밀번호 관리 도구)에서 전한다.
