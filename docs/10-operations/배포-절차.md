...
**마지막 배포: 2026-08-27 커밋 `fd94dba`** (모니터링 스택까지. 정산 시드 적재는
2026-08-22 에 끝냈다 — `seed.js` 로 부대비 유형과 테넌트 사업자등록번호를 채운 뒤
`seed-demo.js --reset`). 그 뒤 dev 에 쌓인 것은 문서와 테스트뿐이라 **가동계와
코드 동작은 같다.**
