...
배포는 [배포-절차.md](./배포-절차.md) 에 있다.

Grafana 는 **다른 장비**에 있다 (`grafana.aitestbed.kr` = 180.210.77.62).
다만 실제로 들어오는 출발지는 **180.210.76.21** 이다(NAT). nginx 는 둘 다
열어 두었다 — `docker/nginx/conf.d/ntms.conf`.
...
- **basic auth 를 일부러 안 걸었다.** 평문 HTTP 라 인증을 걸면 15초마다
  ...
  그때 얹는다.
- **Grafana 장비의 아웃바운드 IP 가 A 레코드와 다르다**(NAT — 위의 76.21).
  이미 열어 두었으므로 지금은 통하지만, 그쪽 NAT 이 바뀌면 다시 403 이 된다.
  그때는 실제 출발지를 보고 `allow` 를 더한다. **access.log 를 grep 하지
