...
> 최종 수정: 2026-09-16 · 기능 마지막 진전: 2026-08-22 (정산 — 파이프라인이 닫혔다)
...
**가동계는 2026-09-15 에 최신 코드(`6857124`)로 갱신했다.** 그전까지 서버는 `git pull`
만 받고 다시 빌드하지 않아 **3주 동안 8월 22일 이미지로 돌고 있었다.** 서버의 `git log`
는 앞서 있어서 이것이 보이지 않는다 — 무엇이 돌고 있는지는 컨테이너 생성 시각으로
묻는다(`docker inspect -f '{{.Created}}' ntms-api`).
[`10-operations/배포-따라하기.md`](10-operations/배포-따라하기.md) 가 그 순서다.
